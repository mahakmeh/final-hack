document.addEventListener('DOMContentLoaded', async () => {
    ...
    async function initializeGame() {
        ...
        // Fetch first sentence with error handling
        let targetSentence;
        try {
            targetSentence = await fetchRandomSentence();
            if (!targetSentence) {
                throw new Error('No sentence fetched');
            }
        } catch (error) {
            console.error('Failed to fetch initial sentence:', error);
            targetSentence = "Default fallback sentence for error handling";
        }
        ...
        card_p.addEventListener('input', async (e) => {
            ...
            // Add a null check for currentTargetSentence
            if (!currentTargetSentence) {
                console.error('No target sentence available');
                return;
            }
...
function endGame() {
    ...
    startNewGameButton.style.marginTop = '20px';
    startNewGameButton.style.padding = '10px 20px';
    startNewGameButton.style.fontSize = '16px';
    startNewGameButton.addEventListener('click', initializeGame);

    // Append the button to the card
    card_p.appendChild(startNewGameButton);
}

// Function to initialize the game (called when the button is clicked)
async function initializeGame() {
    // Reset game state
    gameState = 'ready';
    timeRemaining = totalGameTime;
...
// Start the game by calling initializeGame when the page loads
initializeGame();
